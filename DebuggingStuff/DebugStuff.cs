    ...
    internal class DebugStuff : MonoBehaviour
    {
        ...
        private void DumpGameObjectChilds(GameObject go, string pre, StringBuilder sb)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (!child.GetComponent<Transform>() && child.name != "main camera pivot")
        ...
        private GameObject CheckForPartUnderCursor()
        {
            ...
            {
				return GameObject.Find("model_vab_interior_props_v16");

                //return null;
            }
			GameObject componentUpwards = hit.collider.gameObject.GetComponentUpwards<Transform>().gameObject;
